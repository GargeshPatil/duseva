...
export function useExamEngine(testId: string) {
    ...
    const performSubmission = async () => {
        ...
        if (success) {
            ...
            // router is called by handleAutoSubmit or user click wrapper
        } else {
    ...
    const handleAutoSubmit = useCallback(async () => {
        ...
        if (attemptId) {
             router.push(`/dashboard/analysis/${attemptId}`);
        }
    }, [attemptId, performSubmission, router]);

    useEffect(() => {
        if (isTestStarted && timeRemaining <= 0 && attemptId) {
            handleAutoSubmit();
        }
    }, [timeRemaining, isTestStarted, attemptId, handleAutoSubmit]);
