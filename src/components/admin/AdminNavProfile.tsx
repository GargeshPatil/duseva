...
export function AdminNavProfile() {
    ...
    return (
        <div className="relative" ref={profileRef}>
            ...
            >
                <div className="h-8 w-8 rounded-full bg-gradient-to-br from-rose-500 to-orange-500 flex items-center justify-center text-sm font-bold text-white shadow-inner border border-white/20">
                    {initials}
                </div>
                <div className="hidden lg:flex flex-col items-start text-left">
                    <span className="text-sm font-semibold text-white leading-tight flex items-center gap-2">
                        {userData?.name || 'Admin'}
                        <span className="text-[10px] bg-semantic-error/20 text-semantic-error px-1.5 py-0.5 rounded-sm line-height-none border border-semantic-error/30">
                            {userData?.role || 'admin'}
                        </span>
                    </span>
                    <span className="text-[10px] text-text-muted font-medium truncate max-w-[120px]">
                        {userData?.email || 'Admin Account'}
                    </span>
                </div>
                ...
                {isProfileOpen && (
                    ...
                    >
                        <div className="p-4 border-b border-white/5 bg-white/5">
                            <p className="text-sm font-bold text-white truncate text-semantic-error">{userData?.name || 'Administrator'}</p>
