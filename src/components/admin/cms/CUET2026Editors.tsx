...
export function SyllabusEditor({ value, onChange, disabled }: { value: string, onChange: (v: string) => void, disabled: boolean }) {
    // If the old format (coreTopics) is present, we should gracefully handle it or default.
    let baseItems = safelyParseJSON<any[]>(value, []);
    const items: SyllabusSubject[] = Array.isArray(baseItems) ? baseItems.map(item => {
        // Migration from old coreTopics flat array
        if (item.coreTopics && !item.topics) {
            return {
                ...item,
                topics: item.coreTopics.map((t: string) => ({ title: t, subtopics: [] }))
            };
        }
        return {
            ...item,
    ...
    return (
        <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="space-y-3 mt-2">
                                {item.topics.map((topic, tIdx) => (
                                    <div key={tIdx} className="flex gap-3 bg-white border border-slate-200 p-3 rounded-lg">
                                        <div className="flex-1 space-y-2">
                                            ...
                                            <textarea
                                                ...
                                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm text-slate-700 outline-none focus:ring-1 focus:ring-blue-500 min-h-[40px] resize-y"
                                            />
                                        </div>
            ...
            <Button disabled={disabled} onClick={addItem} variant="outline" size="sm" className="w-full border-dashed"><Plus className="h-4 w-4 mr-2" /> Add Subject</Button>
        </div>
    );
}
