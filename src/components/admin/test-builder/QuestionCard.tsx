...
export function QuestionCard({ question, index, onChange, onDelete, onDuplicate }: QuestionCardProps) {
    ...
    return (
        <Reorder.Item value={question} id={question.id} dragListener={false} dragControls={dragControls}>
            <div className="bg-surface-card rounded-xl shadow-sm border border-border p-6 relative group transition-all hover:border-cta-primary/50">
                {/* Drag Handle & Header */}
                <div className="absolute left-2 top-6 cursor-move text-text-muted hover:text-text-secondary p-2 touch-none" onPointerDown={(e) => dragControls.start(e)}>
                ...
                <div className="pl-8 space-y-4">
                    <div className="flex justify-between items-start gap-4">
                        <div className="flex-1">
                            ...
                            <textarea
                                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm outline-none focus:ring-2 focus:ring-ring transition-all min-h-[60px] resize-y"
                                value={question.text}
                    ...
                    <div className="space-y-2">
                        {question.options.map((opt, i) => (
                            <div key={i} className="flex items-center gap-3">
                                ...
                                <Input
                                    ...
                                    className={`flex-1 ${question.correctOption === i ? 'bg-semantic-success/10 border-semantic-success/30' : ''}`}
                                />
                        ...
                        {question.options.length < 6 && (
                            <div className="flex items-center gap-3 pl-8">
                                <button
                                    onClick={addOption}
                                    className="text-sm text-cta-primary hover:text-cta-hover font-medium py-1 px-2 hover:bg-cta-primary/10 rounded"
                                >
                                    + Add Option
                                </button>
                            </div>
                    ...
                    <div className="pt-4 border-t border-border grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-medium text-text-muted mb-1">Explanation (Solution)</label>
                            <textarea
                                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-xs outline-none focus:ring-1 focus:ring-ring min-h-[60px]"
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select
                                    className="w-full px-2 py-1.5 bg-surface-base border border-border rounded text-xs"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-text-muted mb-1">Stream</label>
                                <select
                                    className="w-full px-2 py-1.5 bg-surface-base border border-border rounded text-xs"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-text-muted mb-1">Category (Subject)</label>
                                <Input
