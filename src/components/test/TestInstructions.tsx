...
export function TestInstructions({
    ...
    return (
        <div className="flex flex-col h-screen bg-background">
            {/* Header */}
            <header className="h-16 bg-surface-card border-b border-border text-text-primary flex items-center px-6 shadow-sm shrink-0">
                <h1 className="text-xl font-bold">General Instructions</h1>
            </header>

            {/* Content */}
            <main className="flex-1 overflow-y-auto p-6 md:p-10 bg-background">
                <div className="max-w-4xl mx-auto bg-surface-card p-8 rounded-xl shadow-sm border border-border">
                    <h2 className="text-2xl font-bold text-text-primary mb-6 border-b border-border pb-4">
                        {testTitle}
                    </h2>

                    <div className="flex flex-wrap gap-6 mb-8 text-sm font-medium text-text-primary bg-surface-elevated p-4 rounded-lg border border-border">
                        <div className="flex items-center gap-2">
                            <span className="text-text-muted">Duration:</span>
                            <span className="text-text-primary">{durationMinutes} Minutes</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-text-muted">Maximum Marks:</span>
                            <span className="text-text-primary">{totalMarks}</span>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="space-y-6 text-text-secondary leading-relaxed">
                        <section>
                            <h3 className="font-bold text-text-primary mb-3 text-lg">1. General Instructions:</h3>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4 mb-6">
                                <div className="flex items-center gap-2 text-sm">
                                    <div className="w-8 h-8 flex items-center justify-center bg-surface-elevated border border-border rounded text-text-secondary font-medium">1</div>
                                ...
                                <div className="flex items-center gap-2 text-sm">
                                    ...
                                    <span className="text-text-secondary">Answered & Marked for Review</span>
                                </div>
                            </div>
                        </section>

                        <section>
                            <h3 className="font-bold text-text-primary mb-3 text-lg">2. Navigating to a Question:</h3>
                            <ul className="list-disc pl-5 space-y-2">
                        ...
                        <section>
                            <h3 className="font-bold text-text-primary mb-3 text-lg">3. Answering a Question:</h3>
                            <ul className="list-disc pl-5 space-y-2">
                        ...
                        <div className="bg-semantic-warning/10 border-l-4 border-semantic-warning p-4 mt-6">
                            <div className="flex gap-3">
                                <AlertTriangle className="h-5 w-5 text-semantic-warning shrink-0 mt-0.5" />
                                <p className="text-sm text-text-secondary">
                                    <strong className="text-text-primary">Note:</strong> Questions marked for review will trigger a warning before submission, but answers marked for review WILL BE considered for evaluation if left unmarked.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            {/* Footer Action */}
            <footer className="bg-surface-card border-t border-border flex flex-col md:flex-row items-center justify-between px-6 md:px-10 py-4 md:py-0 md:h-20 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] gap-4">
                <div className="flex items-start md:items-center gap-3 w-full md:w-auto">
                    <input
                        ...
                        className="mt-1 md:mt-0 h-5 w-5 rounded border-border text-cta-primary focus:ring-cta-primary cursor-pointer bg-surface-base"
                    />
                    <label htmlFor="agree" className="text-sm md:text-base font-medium text-text-primary cursor-pointer select-none leading-tight">
                ...
                <Button
                    ...
                    className={`
                        ...
                        ${agreed ? "bg-cta-primary hover:bg-cta-hover shadow-lg shadow-cta-primary/20 text-white" : "bg-surface-elevated text-text-muted cursor-not-allowed border border-border"}
                    `}
                >
